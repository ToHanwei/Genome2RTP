...
def unredundant(outputdir, prefix, func_file, pseu_file, hitdna, EvalueLimit, cpus, profile, verbose):
    """
    Function:
        unredundant
    ...
    if not verbose:
        pseucomm += ' > /dev/null'
    os.system(pseucomm)
    func = dict(ReadSampleFasta(nonredun_func))
    dna = dict(ReadSampleFasta(nonredun_dna))
    pseu = ReadSampleFasta(nonredun_pseu)
    # Exclude ammonia receptors
    hmm_len = getHMMlen(profile)
    basename = os.path.split(hitdna)[1]#os.path.split()：按照路径将文件名和路径分割开os.path.split()：按照路径将文件名和路径分割开
    tempname = "/tmp/"+ basename + str(time.time())
    run_nhmmer(EvalueLimit, cpus, tempname, profile, hitdna, verbose)
    #os.remove(tempname)
    taardict = proc_aminergic(tempname)
    taardict = merge_hit(taardict, hmm_len)
    excludes = exclude_aminergic(
                    EvalueLimit,
                    cpus,
                    "/tmp/" + basename,
                    nonredun_dna,
    ...
    for exc in excludes:
        del func[exc+'\n']
        del dna[exc+'\n']
    Writer_dict(func, nonredun_func)
    Writer_dict(dna, nonredun_dna)

    logging.info('Exclude {} ammonia receptors'.format(len(excludes)))
...
def Writer_dict(indict, filename):
    """
    Function:
        Writer_dict
        write dict to file
    Parameter:
        indict: dict
        filename: string, write out file name
    Return: None
    """
    fo = open(filename, 'w')
...
def exclude_aminergic(evalue, cpus, prefix, seqfile, hmmlen, taardict, verbose):
    """
    Function:
        exclude_aminergic
        use HMMER exclude aminergic sequence
    Parameter:
        evalue, report sequences <= this E-value threshold in output
        cpus,  number of parallel CPU workers to use
        output, nhmmer output file
        hmmlen: int, the length of HMM profile
        verbose, show version message and exit [true]
        taardict, merge_hit function output
        seqfile: string, annotation output DNA file
    Return:
        clearfile: string, exclude aminergic sequences
    """
    excludes = []
    out_excludes = []
    hmms = os.listdir(AMINEHMM)
    hmms_out = [os.path.join(AMINEHMM, _) for _ in hmms]
    for name, hmm in zip(hmms, hmms_out):
        name = os.path.splitext(name)#将对应路径的文件名和后缀名分割
        name = name[0]
        output = prefix + "_" + name + ".tblout"
        run_nhmmer(evalue, cpus, output, hmm, seqfile, verbose)
        procdict = proc_aminergic(output)
        procdict = merge_hit(procdict, hmmlen)
        if procdict:
            excludes.append(procdict)
        os.remove(output)
    for excds in excludes:
        for exc in excds:
            try:
                sour_evalue = taardict[exc][1]
                excd_evalue = excds[exc][1]
                if sour_evalue > excd_evalue:
                    out_excludes.append(exc)
            except KeyError:
                pass
    return set(out_excludes)


def proc_aminergic(tblout):
    """
    Function:
        proc_aminergic
        process aminergic HMM profile
    Parameter:
        tblout: string, NHMMER outfile format
    Return:
        procdict: dict
        key   --> NHMMER hit name
        value --> extract data
    """
    with open(tblout) as tbloutf:
        # Filter the header and tail
        linelist = [line for line in tbloutf if line[0:1] != '#']
    procdict = defaultdict(list)#查找字典里不存在的key会报错而使用defaultdict可以避免这个问题
    for line in linelist:
        temp = line.strip().split()
        # Extract information from NHMMER outfile
        sca = temp[0]
        ...
        evalue = float(temp[12])
        if sign == "+":
            procdict[sca].append((hmmfr, hmmto, evalue))
        else:
            # if sign == '-', it's not right
            pass
    return procdict


def merge_hit(procdict, hmmlen):
    """

    """
    newdict = {}
    for hit, data in procdict.items():#items可以遍历key、value
        if len(data) == 1:
            ele = data[0]
            hlen = ele[1] - ele[0] + 1
            evalue = ele[2]
        elif len(data) > 1:
            hlen = sum(ele[1]-ele[0]+1 for ele in data)
            evalue = min(ele[2] for ele in data)
        else:
            print("tblout error")
            sys.exit()
        cover = (100 * hlen) / hmmlen
        cover = round(cover, 2)
        if cover > COVER_RATE:
            newdict[hit] = (cover, evalue)
    return newdict
...
def getHMMlen(hmm):
    ...
    Function:
        getHMMlen
        get the length of HMM profile
    Parameter:
        hmm: string, HMM profile path
    Return:
        hmm_len: int, length of HMM profile
    """
    with open(hmm) as hmmf:
        lines = hmmf.readlines()[1:10]
    linedict = dict(line.split() for line in lines)
    try:
        hmm_len = int(linedict["LENG"])
    except:
        print("HMM profile error")
        sys.exit()
    return hmm_len
